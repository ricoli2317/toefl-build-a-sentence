...
test("C3 is the default while legacy remains an explicit rollback with independent deadlines", () => {
  ...
  assert.deepEqual(writingReviewPipelineTiming("c3", {}), { hedgeDelayMs: 60_000, deadlineMs: 120_000 });
...
test("invalid pipeline and invalid C3 deadline cannot silently change behavior", () => {
  ...
  assert.deepEqual(writingReviewPipelineTiming("c3", { WRITING_REVIEW_C3_DEADLINE_MS: "60000" }), { hedgeDelayMs: 60_000, deadlineMs: 120_000 });
});
