...
test("C3 is the default while legacy remains an explicit rollback with independent deadlines", () => {
  ...
  assert.deepEqual(writingReviewPipelineTiming("c3", {}), { hedgeDelayMs: 90_000, deadlineMs: 210_000 });
  assert.deepEqual(writingReviewPipelineTiming("c3", { WRITING_REVIEW_C3_DEADLINE_MS: "210000" }), { hedgeDelayMs: 90_000, deadlineMs: 210_000 });
});
...
test("invalid pipeline and invalid C3 deadline cannot silently change behavior", () => {
  ...
  assert.deepEqual(writingReviewPipelineTiming("c3", { WRITING_REVIEW_C3_DEADLINE_MS: "60000" }), { hedgeDelayMs: 90_000, deadlineMs: 210_000 });
  assert.deepEqual(writingReviewPipelineTiming("c3", { WRITING_REVIEW_C3_DEADLINE_MS: "180000" }), { hedgeDelayMs: 90_000, deadlineMs: 210_000 });
});
