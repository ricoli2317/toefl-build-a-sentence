# CTW Logical Identity Full-Database Audit

Generated: 2026-09-13T09:56:48.436Z

This is a read-only factual manifest. It does not choose a canonical survivor or canonical prefix.

## Summary

- Total CTW logical items: 166
- Total CTW occurrences audited: 304
- New logical identities: 166
- Duplicate clusters: 0
- Logical items in duplicate clusters: 0
- Theoretical logical-item reduction: 0
- Prefix-conflict clusters: 0
- Punctuation/rendering-only clusters: 0
- Clusters with duplicate occurrence mappings: 0
- Clusters with other manual-review differences: 0

## Known access/resources case

- Status: single_database_version
- Matching logical items: 1
- `ac____` version present: true
- `acc___` same-passage version present: false
- `resources such` version present: true
- `resources, such` same-passage version present: false

- `reading-ctw-e12835c96f7207294b51eadd`: access prefix `ac`, resources display `reso_ _ _ _ _`; sources 6.21A

