...
import type { WritingReviewDeepSeekStabilityInput } from "../lib/writingReviewDeepSeekStabilityBenchmark.ts";
...
type BenchmarkModule =
  typeof import("../lib/writingReviewDeepSeekStabilityBenchmark");

void run().catch((error) => {
  console.error(
    `DeepSeek stability benchmark stopped: ${
...
async function run() {
  ...
  requireEnvironment();
  const benchmarkPath = "../lib/writingReviewDeepSeekStabilityBenchmark.ts";
  const benchmark = (await import(benchmarkPath)) as BenchmarkModule;

  const existingAdWeak = benchmark.readWritingReviewDeepSeekExistingAdWeak(
    resolve(
      process.cwd(),
      benchmark.WRITING_REVIEW_DEEPSEEK_STABILITY_EXISTING_PRO_PATH
    )
  );
  if (!existingAdWeak) {
    throw new Error(
      "The existing DeepSeek Pro high ad_weak result is missing or incompatible."
    );
  }
  ...
  const [source, supabaseServer, openRouter, schema] = await Promise.all([
  ...
  const inputs: WritingReviewDeepSeekStabilityInput[] = [];
  for (const benchmarkCase of benchmark.WRITING_REVIEW_DEEPSEEK_STABILITY_NEW_CASES) {
  ...
  const newResults = await benchmark.benchmarkWritingReviewDeepSeekStability(
    inputs,
    {
        ...
        openRouter.requestOpenRouterWritingReview(input, {
          jsonSchema:
            schema.AI_REVIEW_RAW_RESULT_V22_JSON_SCHEMA as unknown as Record<
              ...
              unknown
            >,
          modelOverride: benchmark.WRITING_REVIEW_DEEPSEEK_STABILITY_MODEL,
          reasoningEffort: benchmark.WRITING_REVIEW_DEEPSEEK_STABILITY_EFFORT,
          signal
        }),
  ...
  const allResults = [...newResults, existingAdWeak];
  const baselines = benchmark.WRITING_REVIEW_DEEPSEEK_STABILITY_ALL_CASES.map(
    (benchmarkCase) =>
      benchmark.readWritingReviewDeepSeekKimiBaseline(
        benchmarkCase,
        resolve(
          process.cwd(),
          benchmark.WRITING_REVIEW_DEEPSEEK_STABILITY_KIMI_DIR,
  ...
  const outputDir = resolve(
    process.cwd(),
    benchmark.WRITING_REVIEW_DEEPSEEK_STABILITY_OUTPUT_DIR
  );
  benchmark.writeWritingReviewDeepSeekStabilityFiles(
    outputDir,
    allResults,
    baselines
  );
...
function printResultTable(
  ...
  console.log("\nDeepSeek V4 Pro Stability Benchmark\n");
  console.log(
