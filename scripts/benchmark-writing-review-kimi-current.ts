...
import type { WritingReviewKimiCurrentInput } from "../lib/writingReviewKimiCurrentBenchmark.ts";
...
type BenchmarkModule =
  typeof import("../lib/writingReviewKimiCurrentBenchmark");

void run().catch((error) => {
  console.error(
    `Kimi current-prompt benchmark stopped: ${
...
async function run() {
  ...
  const benchmarkPath = "../lib/writingReviewKimiCurrentBenchmark.ts";
  ...
  const inputs: WritingReviewKimiCurrentInput[] = [];
  for (const benchmarkCase of benchmark.WRITING_REVIEW_KIMI_CURRENT_CASES) {
  ...
  const results = await benchmark.benchmarkWritingReviewKimiCurrent(inputs, {
      ...
      openRouter.requestOpenRouterWritingReview(input, {
        jsonSchema:
          schema.AI_REVIEW_RAW_RESULT_V22_JSON_SCHEMA as unknown as Record<
            ...
            unknown
          >,
        modelOverride: benchmark.WRITING_REVIEW_KIMI_CURRENT_MODEL,
  ...
  const historical = benchmark.WRITING_REVIEW_KIMI_CURRENT_CASES.map(
    (benchmarkCase) =>
      benchmark.readWritingReviewKimiHistoricalBaseline(
        benchmarkCase,
        resolve(
          process.cwd(),
          benchmark.WRITING_REVIEW_KIMI_CURRENT_HISTORICAL_DIR,
  ...
  const outputDir = resolve(
    process.cwd(),
    benchmark.WRITING_REVIEW_KIMI_CURRENT_OUTPUT_DIR
  );
  benchmark.writeWritingReviewKimiCurrentFiles(
    outputDir,
    results,
    historical
  );
