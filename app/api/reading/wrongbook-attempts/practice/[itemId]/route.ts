...
/** Full Set correction-only content endpoint; normal Reading practice never uses this route. */
export async function GET(
  ...
  const auth = await profile.measure(
    "wrongbook practice auth",
    [],
    () => requireUserWithRole(bearerToken(request), "student")
  );
  if (auth.error || !auth.userId) {
    return json({ error: "请先登录后再打开错题订正。" }, { status: 401 });
  }

  try {
    const service = createServiceSupabase();
    const practice = await profile.measure(
      "wrongbook practice content loading total",
      ["wrongbook practice auth"],
      () => loadStudentReadingPractice(
    ...
    const response = profile.measureSync(
      "wrongbook practice serialization",
      ["wrongbook practice content loading total"],
      () => json({ practice }),
    ...
    console.error("Full Set wrongbook Reading practice load failed", {
      error,
      itemId: params.itemId
    });
    return json({ error: "阅读错题内容加载失败，请稍后重试。" }, { status: 500 });
  }
}
