...
import { loadTeacherStudentReadingAttemptReview } from "@/lib/teacherStudentReadingAttempt.server";
...
export async function GET(
  ...
  if (!studentId || !attemptId) {
    return json({ error: "无效的阅读练习记录。" }, { status: 400 });
  }
  ...
  try {
    ...
    const detail = await loadTeacherStudentReadingAttemptReview(db, studentId, attemptId);
    if (!detail) return json({ error: "未找到这次阅读练习记录。" }, { status: 404 });
    ...
    console.error("Teacher student Reading attempt load failed", {
      message: error instanceof Error ? error.message : String(error)
    });
    const response = json({ error: "阅读作答加载失败，请稍后重试。" }, { status: 500 });
    return debugEnabled ? appendSupabaseDebugMetrics(response, debugMetrics) : response;
  }
}
