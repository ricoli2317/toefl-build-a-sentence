...
export async function GET(
  ...
  try {
    ...
    if (!detail) return json({ error: "未找到这次阅读订正记录。" }, { status: 404 });
    ...
    console.error("Teacher student Reading wrongbook attempt load failed", {
      message: error instanceof Error ? error.message : String(error)
    });
    const response = json({ error: "阅读订正作答加载失败，请稍后重试。" }, { status: 500 });
    return debugEnabled ? appendSupabaseDebugMetrics(response, debugMetrics) : response;
  }
}
