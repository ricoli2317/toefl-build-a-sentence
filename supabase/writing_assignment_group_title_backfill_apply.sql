-- Apply block extracted verbatim (uncommented) from
-- supabase/writing_assignment_group_title_backfill_dry_run.sql lines 113-190.
-- Run only after the dry-run SELECT output was reviewed and confirmed.
-- It recomputes the same stable numbering and only updates safe rows
-- (can_auto_update = true) whose title is still null.
