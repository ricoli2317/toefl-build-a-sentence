...
function PracticeSetGrid({ sets }: { sets: PracticeSet[] }) {
  if (sets.length === 0) return <StudentEmptyState text="未找到套题。" />;
      ...
      {sets.map((set) => (
        ...
        >
          ...
          <div className="col-start-2 row-start-1 flex min-h-7 items-center justify-end md:col-start-3">
            {set.completed ? (
              <span className="inline-flex min-h-7 items-center rounded-full bg-student-primary-soft px-3 py-1 text-xs font-semibold text-student-primary">
                已完成
              </span>
            ) : (
